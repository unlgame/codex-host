...
## codexhost integration

Use these references when the task actually involves AI chat components. Inspect the owning package and its existing UI before installing anything. The examples are copied reference snippets, not build inputs or already-installed product dependencies. Apply root package exports and Renderer browser-safety rules.

The upstream Next.js, AI SDK, shadcn and gateway setup guidance is conditional on a project using that stack. codexhost's existing Renderer extension does not become a Next.js app by using this skill. Select commands from the target package's scripts and package manager, reuse existing components where suitable, and add dependencies only when needed for the authorized implementation.
