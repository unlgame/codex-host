...
## codexhost test tooling

Use the browser-control tools available in this session via [control-browser](../control-browser/SKILL.md). The `agent-browser` command examples below are for the CLI route only; use the selected runtime's documented equivalents. If video recording is unavailable, report that limitation and retain screenshots plus reproduction steps rather than claiming a recording exists.

Preserve existing user authorization. Login or test setup does not authorize unrelated external writes. Test scope determines which actions may be exercised; record blocked cases honestly. Use [web-gui-tester](../web-gui-tester/SKILL.md) for explicit interaction and visual verification cases.
...
See ../NOTICE.md for retained license and provenance information.
-->
...
When using the CLI route, prefer the installed `agent-browser` executable over `npx agent-browser`. The direct binary uses the fast Rust client. `npx` routes through Node.js and is significantly slower.
