...
## codexhost application routing

Honor the user's target app. Prefer available native app/browser-control tools and their documentation; the CLI/CDP examples below apply when that is the selected, available route. For browser runtime selection see [control-browser](../control-browser/SKILL.md).

For codexhost source launches follow root AGENTS.md and the current `npm start` implementation. Launching can stop running Codex Desktop processes; do it when launch is part of the task, not as a routine check. Reuse existing Desktop control and versioned binding modules when writing repository code. Do not assume every packaged Electron app permits remote debugging or that a port is already enabled; inspect the actual target.
...
A target may expose `--remote-debugging-port` through Chromium; packaged builds can restrict it. Verify support before connecting.
