...
## codexhost tool routing

Use this guide when the selected tool is the `agent-browser` CLI. First verify its availability and `--help`; copying the skill does not install the executable. If the session provides browser-control tools instead, use [control-browser](../control-browser/SKILL.md) and their actual API documentation rather than translating CLI examples into invented calls. Honor the user's browser choice. CLI commands and shell templates below apply only to the CLI route.
